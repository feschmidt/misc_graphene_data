...
def fitmodel(f_0,
    ...
    # kintfix=False: fit kappaint as exponential
    """

    f_p = Probe_freq
    #if kintfix:
    k = kint + kext  # TODO: add power-dependence here for nonlinear damping
    #else:
...
class MyWrapper():
    ...
    """
    def __init__(self, classparams, ikint=0,bgremoval=True,trimming=True,dev='2x2',devdf=None):
        ...
        self.mymodel.load_data(ikint=ikint,bgremoval=bgremoval,dev=dev,devdf=devdf)

        self.X2, self.Y2, self.DAT2 = self.mymodel.inspect_measurement_data(trimming=trimming)
